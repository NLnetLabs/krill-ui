...
test('Expanding the details of a ROA', async ({ page }) => {
  ...
  const numOfRoas = await page
    .locator('table tbody tr td button.expand')
    .count();
  ...
  for (let idx = 0; idx < numOfRoas; idx++) {
    ...
    await expect(
      page.locator('table tbody tr.announcements td div.row')
    ).toHaveCount(1);

    const inspectedRow = await page.locator('table tbody tr', {
      has: page.locator('button.expand.open'),
    });

    {
      ...
      const authorizesDiv = await page.locator('tr div.row div', {
        hasText: /Authorizes \d* announcements/,
      });
      const headingAuthorized = await authorizesDiv.locator('h3').innerText();
      const numberAuthorized = parseInt(
        headingAuthorized.substring(10, headingAuthorized.length - 13)
      );

      expect(badgeNumberAuthorized).toBe(numberAuthorized);

      if (badgeNumberAuthorized === 0) {
        expect(await authorizesDiv.locator('table tbody tr').innerText()).toBe(
          'No data'
        );
      } else {
        const numberOfDisallowedRows = await authorizesDiv
...

    {
      ...
      const disallowDiv = await page.locator('tr div.row div', {
        hasText: /Disallows \d announcements/,
      });
      const headingDisallowed = await disallowDiv.locator('h3').innerText();
      const numberDisallowed = parseInt(
        headingDisallowed.substring(9, headingDisallowed.length - 13)
      );

      expect(badgeNumberDisallowed).toBe(numberDisallowed);

      if (badgeNumberDisallowed === 0) {
        expect(await disallowDiv.locator('table tbody tr').innerText()).toBe(
          'No data'
        );
      } else {
        const numberOfDisallowedRows = await disallowDiv
          .locator('table tbody tr')
          .count();
