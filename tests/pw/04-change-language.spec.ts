...
test('change languages', async ({ page }) => {
  ...
  await expect(userButton.locator('div', { hasText: 'Benutzer:' })).toHaveCount(
    1
  );

  await expect(
    page.locator('h2', { hasText: 'Zertifizierungsstelle' })
  ).toHaveCount(1);
  await expect(
    page.locator('label', { hasText: 'Aktuelle Zertifizierungsstelle' })
  ).toHaveCount(1);
  ...
  await expect(page.locator('a', { hasText: 'Support Verträge' })).toHaveCount(
    1
  );
  await expect(
    page.locator('a', { hasText: 'Lesen Sie die Dokumentation' })
  ).toHaveCount(1);
  await expect(
    page.locator('a', { hasText: 'Ein Problem melden' })
  ).toHaveCount(1);
  ...
  if ((await disallowing.count()) > 0) {
    await expect(await disallowing.first().innerText()).toBe(
      'Verursacht ungültige Ankündigungen'
    );
  }

  const invalidLength = page.locator('span.state.announcement_invalid_length');
  if ((await invalidLength.count()) > 0) {
    await expect(await invalidLength.first().innerText()).toBe(
      'UNGÜLTIGE LÄNGE'
    );
  }
  ...
  await expect(
    page.locator('h3', { hasText: /Autorisiert \d* BGP-Ankündigungen/ })
  ).toHaveCount(1);
  await expect(
    page.locator('h3', { hasText: /Verweigert \d* BGP-Ankündigungen/ })
  ).toHaveCount(1);
});
