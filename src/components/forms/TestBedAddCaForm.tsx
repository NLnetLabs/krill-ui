...
export default function TestBedAddCaForm() {
  ...
  const verifyChildXML = (doc: Document): boolean => {
    ...
    if (
      // @ts-ignore
      !doc.getElementsByTagName('child_request')[0].attributes['child_handle']
    ) {
      setNotification({
        type: NotificationType.error,
    ...
    if (doc.getElementsByTagName('child_bpki_ta')[0].childNodes.length === 0) {
      setNotification({
        type: NotificationType.error,
        message: t.testbed.errors.empty_xml_el.replace('{el}', 'child_bpki_ta'),
      });
      return false;
    }
    ...
    if (
      // @ts-ignore
      doc
        .getElementsByTagName('child_bpki_ta')[0]
    ...
    return {
      handle:
        ...
        doc.getElementsByTagName('child_request')[0].attributes['child_handle']
          .value,
      id_cert: (
        doc.getElementsByTagName('child_bpki_ta')[0].childNodes[0]
          .nodeValue as string
      ).trim(),
    };
  };
      ...
      <form onSubmit={onSubmit} method="POST">
        <div>
          <label>
            ...
            <a href="https://tools.ietf.org/html/rfc8183#section-5.2.1">
